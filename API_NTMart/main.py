...
def reset_password(payload: ResetPasswordRequest) -> dict[str, str]:
    ...
    try:
        ...
        phone_from_token = decoded_token.get("phone_number")  # VD: +84xxxxxxxxx
    except Exception:
        raise HTTPException(status_code=401, detail="Token không hợp lệ hoặc đã hết hạn")

    # Chuyển +84xxxxxxxxx → 0xxxxxxxxx để so khớp với DB
    phone_normalized = "0" + phone_from_token[3:] if phone_from_token.startswith("+84") else phone_from_token

    # Kiểm tra số điện thoại khớp với token
    if phone_normalized != payload.phone:
        raise HTTPException(status_code=403, detail="Số điện thoại không khớp")

    # Tìm user theo số điện thoại và đổi mật khẩu
    with closing(get_connection()) as connection:
        user = fetch_one(
            connection,
        ...
        connection.execute(
            ...
            (payload.new_password, phone_normalized),
        )
        connection.commit()
